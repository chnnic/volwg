...
新建多节点时，留空节点 ID 会自动选择 `home1`、`home2` 等未使用 ID；自动编号会同时检查活动接口、WireGuard 配置和 VolWG 已登记线路。节点 ID 是用于网卡、服务、配置文件和 Xray tag 的内部标识，只允许 1-8 位小写字母、数字或下划线；线路显示名称可以自由使用大写字母、连字符和中文。交互输入不合规时不会退出向导，而会给出规范化建议并停留在当前步骤重新输入。VPS/家宽两端会在显示端口输入框之前扫描 WireGuard 接口、系统监听端口和全部 VolWG 线路记录；端口已占用时，输入框会直接显示向后找到的可用端口。用户手动修改端口后，写入前还会再次检查。
...
把 VPS 的一行配对码完整粘贴到家宽窗口，家宽端会自动读取所有参数和 VPS 公钥，不再逐项手抄端口或密钥。家宽窗口会单独框出自己的 WireGuard 公钥，并明确提示把完整一行复制回正在等待的 VPS 窗口；交互模式会等待确认已复制后再继续。两边都完成写入后 WireGuard 才会建立连接。家宽机会主动发起连接；整个过程不会要求家宽公网 IP、家宽 SSH 地址或 SSH 私钥路径。仍可留空配对码进入旧的逐项手动填写模式。
